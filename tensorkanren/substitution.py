...
class Substitution(temp_dict):
    ...
    def unify(self, rel_tensor, *args):
        ...
        if ndim0 < ndim1:
            newdim = ndim1
            newsub.result = tf.expand_dims(newsub.result, list(range(ndim0, ndim1)))
    ...
    def _sub_or(self, other):
        ...
        if n1 > n2:
            ...
            r2 = tf.expand_dims(r2, range(n2, n1))
        elif n1 < n2:
            newsub.var_indices.update(other.var_indices)
            r1 = tf.expand_dims(r1, range(n1, n2))
    ...
    def reduce(self, *args, op=tf.reduce_any):
        dims = self.get_var_indices(args)
        dims_sort_ind = tf.argsort(dims)
        dims_trans = tf.zeros(len(dims_sort_ind), int)
        dims_trans[dims_sort_ind] = tf.arange(len(dims_sort_ind))
        return op(self.result, dims).transpose(dims_trans)

    def reduce_to(self, *args, op=tf.reduce_any):
        if not args:
    ...
    def filter(self, args, data):
        ...
        for i in ind:
            exp_ind.remove(i)
        data = tf.expand_dims(data, exp_ind)
